...
let string_of_macro_tokens tokens =
  List.map string_of_macro_token tokens |> String.concat ""
;;
...
let rec print_c_type = function
  ...
  | Array (ct, dims) ->
    let dims_str =
      List.map (fun i -> "[" ^ string_of_int i ^ "]") dims |> String.concat ""
    in
    print_c_type ct ^ dims_str
  | Function (ret, args) ->
    let args_str = List.map print_c_type args |> String.concat ", " in
    print_c_type ret ^ "(" ^ args_str ^ ")"
  | Struct (Some id, fields) -> "struct " ^ id ^ " { " ^ print_fields fields ^ " }"
  | Struct (None, fields) -> "struct { " ^ print_fields fields ^ " }"
  | Union (Some id, fields) -> "union " ^ id ^ " { " ^ print_fields fields ^ " }"
  | Union (None, fields) -> "union { " ^ print_fields fields ^ " }"
  | Enum (Some id, values) -> "enum " ^ id ^ " { " ^ String.concat ", " values ^ " }"
  | Enum (None, values) -> "enum { " ^ String.concat ", " values ^ " }"
