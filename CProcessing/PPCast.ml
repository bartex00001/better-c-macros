...
let rec pp_macro_result_element fmt = function
  ...
  | MacroResUse (id, tokens) ->
    Format.fprintf
      ...
      id
      (Format.pp_print_list pp_macro_result_element)
      tokens
;;

let rec pp_macro_result fmt = function
  | BasicRes elem -> Format.fprintf fmt "BasicRes(%a)" pp_macro_result_element elem
  | SequenceRes elems ->
    Format.fprintf
      fmt
      "SequenceRes(%a)"
  ...
  | MacroRes (id, tokens) ->
    Format.fprintf
      ...
      Format.pp_print_string
      id
      (Format.pp_print_list pp_macro_result)
      tokens
;;

let pp_macro_matcher_element fmt = function
  | DirectMatch token -> Format.fprintf fmt "DirectMatch(%a)" pp_macro_token token
  | NamedMatch (id, mt) ->
    Format.fprintf
      fmt
      "NamedMatch(%a, %a)"
...
let pp_macro_matcher fmt = function
  | BasicMatch elem -> Format.fprintf fmt "BasicMatch(%a)" pp_macro_matcher_element elem
  | SequenceMatch elems ->
    Format.fprintf
      fmt
      "SequenceMatch(%a)"
      (Format.pp_print_list pp_macro_matcher_element)
      elems
;;
...
let rec pp_macro_use fmt (id, result_tokens) =
  Format.fprintf
    ...
    Format.pp_print_string
    id
    (Format.pp_print_list pp_macro_token_result)
    result_tokens
       ...
       Format.fprintf
         ...
         (Format.pp_print_list (fun fmt (id, tokens) ->
            Format.fprintf
              ...
              Format.pp_print_string
              id
              (Format.pp_print_option pp_macro_token)
              tokens))
         attributes))
    fields
  ...
  | Struct (id, fields) ->
    Format.fprintf
      ...
      (Format.pp_print_list (fun fmt (ctype, id) ->
         Format.fprintf fmt "{%a, %a}" pp_ctype ctype Format.pp_print_string id))
      fields
  | Union (id, fields) ->
    Format.fprintf
      fmt
...
let pp_c_elem fmt = function
  ...
  | MacroDef { name; matches } ->
    Format.fprintf
      ...
      Format.pp_print_string
      name
      (Format.pp_print_list (fun fmt pattern ->
         Format.fprintf
           fmt
           "{ matcher = %a,\n result = %a}"
           (Format.pp_print_list pp_macro_matcher)
           pattern.matcher
           (Format.pp_print_list pp_macro_result)
           pattern.result))
      matches
  | MacroUse muse -> Format.fprintf fmt "MacroUse(%a)" pp_macro_use muse
