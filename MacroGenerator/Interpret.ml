...
(** State of the state-machine consists of:
  * - [position] current state id
...
(** - [SMGoto state, tokens_left] represents action of transitioning
  *   to [state] with [tokens_left] yet to be processed
  * -[SMFinish state] represents the state of automaton when end block was found
  * -[SMFail] invalid state reached (would be reached if not stopped) *)
type sm_action =
...
(** Runst the state machine [sm] with [token_list]
  * returns [Some sm_state] if automaton reaches accepting state, [None] otherwise *)
let run_state_machine sm token_list =
  
  let reverse_named_env_lists {position; env; buffer} =
    let new_env = StrMap.map begin function
      | Single t -> Single t
  ...
  let rec run sm_state token_list =
    let state = sm_state.position
    in
    match StateMachine.find_opt state sm with
    | Some transition ->
      (* TODO: Check if there are tokens left *)
      begin match transition (sm_state, List.hd token_list, List.tl token_list) with
...
(** Helper types for constructing state transitions.
  * - [Goto id] go to [id] 
...
let state_transition_of_transition_helper_direct = function
  ...
  | SequenceConfirm next_state -> fun (sm_state, _, token_list) ->
    let new_env = apply_buffer sm_state.env sm_state.buffer
    in
    SMGoto (
      { position = next_state
      ; env = new_env
  ...
  | SequenceFail next_state -> fun (sm_state, t, token_list) -> 
    let clean_buffer = List.map (fun (t, _) -> t) sm_state.buffer
    in
    SMGoto (
      { position = next_state
      ; env = sm_state.env
...
let state_transition_of_transition_helper_named name = function
  | Goto next_state -> fun (sm_state, t,  token_list) ->
    (* TODO: Replace with update and check if this is the only assignment *)
    let new_env = StrMap.add name (Single t) sm_state.env
    in
    SMGoto (
      { position = next_state
      ; env = new_env
  ...
  | SequenceConfirm next_state -> fun (sm_state, t, token_list) ->
    let new_env = apply_buffer sm_state.env ((t, Some name) :: sm_state.buffer)
    in
    SMGoto (
      { position = next_state
      ; env = new_env
...
let get_basic_match_transition (next : transition_helper) (fail : transition_helper) = function
  | DirectMatch(token) -> fun (sm_state, t, token_rest) ->
    let next = state_transition_of_transition_helper_direct next
    and fail = state_transition_of_transition_helper_direct fail
    in
    (if t = token then next else fail)
    (sm_state, t, token_rest)
  | NamedMatch(name, toekn_type) -> fun (sm_state, t, token_rest) ->
    let next = state_transition_of_transition_helper_named name next
    and fail = state_transition_of_transition_helper_named name fail
    in
    begin match toekn_type with
      | TIdent  -> begin function Ident _  -> next | _ -> fail end t
      | TInt    -> begin function Int _    -> next | _ -> fail end t
      | TFloat  -> begin function Float _  -> next | _ -> fail end t
      | TString -> begin function String _ -> next | _ -> fail end t
      | TChar   -> begin function Char _   -> next | _ -> fail end t
      | TExpr   -> begin function
      ...
      | TToken  -> begin function EndToken -> fail | _ -> next end t
    end
    (sm_state, t, token_rest)
...
let add_sequence_match_transition sm start_state after_sequence mme_list (* TODO: type annotate and reduce *) =
  ...
  let rec get_transition sm curr_state = function
    | [] -> sm
    | [mme] -> 
      let transition = get_basic_match_transition start fail mme
      in
...
let[@tail_mod_cons] rec print_result_tokens env = function
  ...
  | BasicRes (NamedRes name) :: tl -> begin match StrMap.find_opt name env with
