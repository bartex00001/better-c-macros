...
let printf_typed prefix sufix what t_name =
  "printf(\"" ^ prefix ^ "\");\n"
  ^ "__" ^ t_name ^ "_debugPrint(" ^ what ^ ", indent+1);\n"
  ^ "printf(\"" ^ sufix ^ "\");\n"
...
let printf_field {name; ctype; attributes} =
  let res =
    ...
    | Tdef t_name, [] ->
      printf_typed (name ^ " = ") " " ("&self->" ^ name) t_name

    | Pointer _, [] -> 
      printf_str (name ^ " = <pointer>;\\n")
  ...
  if res = ""
    then res
    else "_printIndent(indent+1);\n" ^ res


let print_interlude = ref true

let debug {name; fields; typedef} =
  let interlude = if !print_interlude
    then begin
  ...
  in
  let decl = interlude ^ "void __" ^ name ^ "_debugPrint(const struct " ^ name ^ " *self, int indent) {\n"
  in
  ...
  in
  let decl_direct =
    "void " ^ name ^ "_debugPrint(const struct " ^ name ^ " *self){\n"
    ^ "__" ^ name ^ "_debugPrint(self, 0);\n"
    ^ "}\n"
  in
