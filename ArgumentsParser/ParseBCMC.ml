...
let usage_msg = "Usage: bcmc [options] -- <command> [args]\n"
...
let parse_arguments () =
  ...
  let rec match_option = function
    ...
    | "-I" :: path :: tl ->
      ...
      match_option tl
    | "--help" :: _ | "-h" :: _ -> print_endline usage_msg; exit 0
    | _ ->
...
let get_file_name old_file_name =
  Filename.chop_extension old_file_name ^ ".bcm.c"


let extract_files_from_args args =
  let rec get_c_files files_acc args_acc = function
    | [] -> files_acc, args_acc
    | file :: rest -> if Filename.check_suffix file ".c"
      then
        let new_file_name = get_file_name file
        in
...
let parse () =
  let (verbose, include_paths, cmd) = parse_arguments ()
  in
  let (files, args) = match cmd with
