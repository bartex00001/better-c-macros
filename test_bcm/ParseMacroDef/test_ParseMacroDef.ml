...
let parse_channel channel =
  ...
  | CParser.Error ->
    ...
    Printf.eprintf
      "%s:%d:%d-%d: Syntax error: unexpected token '%s'\n"
      p1.pos_fname
      p1.pos_lnum
...
let empty_macro_def =
  root_dir ^ "emptyDef.c", [ MacroDef { name = "myMacro"; matches = [] } ]
;;

let minimal_macro_def =
  ( root_dir ^ "minimal.c"
  , [ MacroDef
        { name = "mySwap"
        ; matches =
...
let minimal_named_matches =
  ( root_dir ^ "minimal_named.c"
  , [ MacroDef
        { name = "named_test"
        ; matches =
...
let basic_sequence_match =
  ( root_dir ^ "basic_sequence_match.c"
  , [ MacroDef
        { name = "sequence"
        ; matches =
            [ { matcher = [ SequenceMatch [ NamedMatch ("x", TToken) ] ]
              ; result = [ SequenceRes [ NamedRes "x"; DirectRes (Direct ",") ] ]
              }
            ]
        }
    ] )
        ...
        ; matches =
            [ { matcher =
              ...
              ; result =
                  ...
                  ; BasicRes (DirectRes (Direct "="))
                  ; SequenceRes [ NamedRes "val"; DirectRes (Direct "+") ]
                  ; BasicRes (DirectRes (Direct ";"))
                  ]
              }
            ; { matcher =
                  [ SequenceMatch [ NamedMatch ("id2", TChar); DirectMatch (Direct "^") ]
              ...
              ; result =
                  ...
                  ; BasicRes (DirectRes (Direct "="))
                  ; BasicRes (DirectRes (Direct "{"))
                  ; SequenceRes [ NamedRes "id2"; DirectRes (Direct ",") ]
...
let macro_expansion_in_res =
  ( root_dir ^ "macro_expansion.c"
  , [ MacroDef
        { name = "counter"
        ; matches =
            [ { matcher = []
              ; result =
                  [ BasicRes (DirectRes (Direct "+")); BasicRes (DirectRes (Int 0)) ]
              }
            ; { matcher =
                  [ BasicMatch (DirectMatch (Ident "a"))
              ...
              ; result =
                  [ BasicRes (DirectRes (Direct "+"))
                  ; BasicRes (DirectRes (Int 1))
                  ; MacroRes ("counter", [ SequenceRes [ NamedRes "rest" ] ])
                  ]
              }
            ; { matcher =
                  [ BasicMatch (DirectMatch (Ident "b"))
  ...
  run
    ...
    [ ( "Empty macro definition parsing"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Empty macro definition" empty_macro_def)
        ] )
    ; ( "Minimal macro definition parsing (one token)"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Minimal macro definition" minimal_macro_def)
        ] )
    ; ( "Minimal named values in macro"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Minimal named matches" minimal_named_matches)
        ] )
    ; ( "Many nested parenthesis"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Parenthesis madness" paren_madness)
        ] )
    ; ( "Basic sequence match"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Basic sequence match" basic_sequence_match)
        ] )
    ; ( "Sequence match"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Sequence match" sequence_match)
        ] )
    ; ( "Macro expansion in result"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Macro expansion in result" macro_expansion_in_res)
        ] )
    ]
;;
