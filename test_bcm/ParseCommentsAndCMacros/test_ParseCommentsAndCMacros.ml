...
let () =
  run
    "C preprocesor and comments removal"
    [ ( "Hello World"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Hello World" parse_hello_world)
        ] )
    ; ( "Smallest Example"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Smallest Example" small_file)
        ] )
    ; ( "Multiline Macro"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Multiline Macro" multiline_macro)
        ] )
    ; ( "Weird Comments"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Weird Comments" weird_comments)
        ] )
    ; ( "Macro in Text"
      , [ test_case
            "check if equal"
            `Quick
            (check_file_parsing "Macro in Text" macro_in_text)
        ] )
    ]
;;
